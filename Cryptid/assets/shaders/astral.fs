...
float bitxor(float val1, float val2)
{
    ...
    for(int i = 1; i < 9; i++) {
        ...
        outp += floor(
            mod(
                mod(floor(val1 * pow(2.0, -f)), pow(2.0, f)) +
                mod(floor(val2 * pow(2.0, -f)), pow(2.0, f)),
            2.0)
        ) * pow(2.0, f);
    }
    return outp / 256.0;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
	...
	float stars = (
    (pNoise(uv * 10.0 + vec2(t / 15.0), 10) * pNoise(uv * 10.0 + vec2(t / 15.0), 10) + 1.5) + 0.15 +
    ((pNoise(uv * 12.0 + vec2(t / 15.0), 10) + 1.2) + 0.3)
) / 2.2 + 0.05 + 0.007 * norm_uv * 1.1;

float clusters = (pNoise(uv * 10.0 - vec2(t / 15.), 10) + 1.5) / 1.5 - 0.25 + 0.007 * norm_uv;

float super_clusters = (pNoise(uv / 15.0, 10) + 0.1) / 2.0 + 0.3 - 0.008 * norm_uv;
