...
// texture_coords = coords of a original_pixel within the atlas texture
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float max_x = (texture_details.x + 1.0) * sprite_width; // max X
    ...
    float res = (q3*q6 - floor(q3*q6))*q4 + (1.0-(q3*q6 - floor(q3*q6)))*(1.0-q4); // transparency

    hsl.x = 0.08;
    hsl.y = 0.7;
    hsl.z = (0.8*hsl.z + res*res*0.2) * (0.3 + uv.y);
    float qalph = 0.66 + res/3.0;
...
{
    if (dissolve < 0.001) {
        return vec4(shadow ? vec3(0.0,0.0,0.0) : final_pixel.xyz, shadow ? final_pixel.a*0.3: final_pixel.a);
    }
    ...
    return vec4(shadow ? vec3(0.0,0.0,0.0) : final_pixel.xyz, res > adjusted_dissolve ? (shadow ? final_pixel.a*0.3: final_pixel.a) : 0.0);
}
